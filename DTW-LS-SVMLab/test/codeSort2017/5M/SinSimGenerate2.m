function [TXT] = SinSimGenerate2(m,n)
Num=m*n;
t=linspace(0,1,Num)*(m*2*pi);
A=sin(sqrt(1.1*t+t.^2));
B=zeros(Num,1);
for i=1:Num
...
for i=2:m
    if rem(i,3)==0
        Temp=Temp+TXT((i-1)*n+10)-TXT((i-1)*n+11);
        TXT((i-1)*n+1:i*n)=TXT((i-1)*n+1:i*n)+Temp;
    else
        Temp=Temp+TXT((i-1)*n+9)-TXT((i-1)*n+10);
        TXT((i-1)*n+1:i*n)=TXT((i-1)*n+1:i*n)+Temp;
    end
end
