...
type='c';
Y_latent=zeros(size(TXTa,1),1);
for i=1:size(TXTa,1)
    index=findCluster(re,TXTa(i,:));
    if index==1
        Y_latent(i) = latentlssvm({S1,Y1,type,Agam,Asig2,'RBF_kernel'},{Aalpha,Ab},TXTa(i,:));
    else
        Y_latent(i) = latentlssvm({S2,Y2,type,Bgam,Bsig2,'RBF_kernel'},{Balpha,Bb},TXTa(i,:));
    end
end
