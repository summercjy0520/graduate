...
laber=re(end,end);
index=find(re(:,end)==laber);
S1=zeros(size(index,1)-1,qq);
 for iii=1:size(index,1)-1
     S1(iii,:)=S(index(iii),:);
 end
% index=find(re(:,end)==laber);
% S1=zeros(size(index,1),qq);
% S2=S;
%  for iii=1:size(index,1)
%      S1(iii,:)=S(index(iii),:);
%      S2(index(iii),:)=zeros(1,qq);
%  end
% S2(ismember(S2,zeros(1,qq),'rows')==1,:)=[];
