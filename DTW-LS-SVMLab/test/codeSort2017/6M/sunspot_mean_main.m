...
TXT= importdata('C:\Documents and Settings\Administrator\桌面\cjy\2017\6\12\SN_m_tot_V2.0.txt');
TXT=TXT(:,4);

TXT=smooth(TXT,30,'lowess');
...
Q=Xtt(1:qq)'-mean(Xtt(1:qq));
Ytra=X(qq+1:end);

for i=1:z
    mean_temp=S(i,:)';
    S(i,:)=S(i,:)-mean(mean_temp);
...
if t-kk>kk
    prediction(jia)=mean(resultT(kk+1:end))+mean(Xtt(1:qq));
else
    prediction(jia)=mean(resultT(1:kk))+mean(Xtt(1:qq));
end
% C0=C0-1;
end
% end
