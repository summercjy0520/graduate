...
dd1=NHDTWnew1219(S,Q);%NHEH0306   NHDTWnew1219
% model = initlssvm(S,Q,'f',[],[],'RBF_kernel',[]);
% dd2=kernel_matrix(S,'RBF_kernel','15.1696',Q);
[k]=FuzzyEntropyT(dd1);
% if jia==22||jia==21
%     k=150;
% end
...
while(isempty(SS)==0)
    ...
    if size(TM,1)>0
        ...
        dtemp=NHEHDTW(TM,Q); %NHDTWnew1219NHEH0306    CosineDistance
%         dtemp=pdist2(TM,Q);
...
prediction(jia) = predictknn({SSA,H','f',gam,sig2,'RBF_kernel'},Q,cjy,1);
% prediction = predictcjy({Xtra,Ytra,'f',gam,sig2,'lin_kernel'},{[X;Xt],Xs},{d,100});
prediction(jia) =prediction(jia) +mean(Q0); %-Q0(1)
end
% end
