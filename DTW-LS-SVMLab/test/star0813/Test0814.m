...
%         YT(t)=1;
        if abs(TT0(t))<1
            ...
            YT(t)=1;
            A1(t)=t;
        elseif abs(TT0(t))>=1&&abs(TT0(t))<2
            TT2(t)=TT0(t);
            YT(t)=2;
            A2(t)=t;
        elseif abs(TT0(t))>=2&&abs(TT0(t))<3
            TT3(t)=TT0(t);
            YT(t)=3;
            A3(t)=t;
        else
...
X=TT(1:h0+zis-2);
% Q=TXT(5591:5600)';
Xt=TT((h0+1+zis-2):H1-1);
XXt=[X;Xt];

Xs=XXt((h0-s-q+zis-1):(h0+zis-s-1));

mm=size(TT(1:(h0-1+zis-1)),1);
...
tempx = predict({XXX,H,'f',gam,sig2,'RBF_kernel'},Xs',1);
 ...
 prediction(zis)=TXT(h0+zis-1)+(TT(h0+zis-2)/tempx);
