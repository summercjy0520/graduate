...
[TXT,TX,RAW] = xlsread('C:\Documents and Settings\Administrator\桌面\cjy\2016\8\data\data\Riverflow.xlsx');
...
%         YT(t)=1;
        if abs(TT(t))<0.02
            ...
            YT(t)=1;
            A1(t)=t;
        elseif abs(TT(t))>=0.02&&abs(TT(t))<0.08
            TT2(t)=TT(t);
            YT(t)=2;
            A2(t)=t;
        elseif abs(TT(t))>=0.08&&abs(TT(t))<0.2
            TT3(t)=TT(t);
            YT(t)=3;
            A3(t)=t;
        else
...
% Ytest1=updown3(TT,S,Xs,qq);
% 
% Ytest11=updown4(TT,S,Xs,qq);
% if Ytest11>=0
%     Ytest11=1;
% else
%     Ytest11=-1;
...
c1=size(index,1);
for j=1:c1
    if index(j)<=qq||index(j)>size(S,1)+qq
        index(j)=100;
    end
end
...
for i=1:ww
    ...
    if diff>=0
        if abs(diff)<0.02
            classR(i)=1;
        elseif abs(diff)>=0.02&&abs(diff)<0.08
            classR(i)=2;
        elseif abs(diff)>=0.08&&abs(diff)<0.2
            classR(i)=3;
        else
            classR(i)=4;
        end
...
ticks=cumsum(ones(32,1));
ts=datenum('01');
tf=datenum('32');
t=linspace(ts,tf,32);
plot(t(1:size(Xs(1:end))),A(1:end),'-*');
hold on;
plot(t(size(Xs(1:end))+1:size(ticks)),Xs,'-r*');
