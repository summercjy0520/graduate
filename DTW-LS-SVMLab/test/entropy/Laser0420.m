...
for i=1:(z-1)
    SSK=[P;S(i,:)];
    csum(i)=cosinecjy(SSK,Q0,'lin_kernel',k0);
end
csum0=csum;

result0=0;
diffrence=0;
while(result0~=Eth&&kc<400)
%||diffrence~=1
...
KK=zeros(k0,1);
[minn mini]=min(csum0);%用于替换，最大值

while (jjk<=k0)
%     jjj=1;%初值
    [maxn maxi]=max(csum0);%找第一个最小值
    if maxi>=qq&&maxi<=(mmm-q-s)
    KK(jjk)=maxi;%记录对应的行
