...
entropy=kentropycjy(KC,'lin_kernel',0); %计算熵，线性核
% TEMP=[TEMP;entropy];
% Eth=max(TEMP)-TEMP(1);%get the difference

if Entropy(end)<Entropy(end-1);%get the difference
   result1=1;
else
   result1=0;
end
if Entropy(2)<Entropy(1);%get the difference
   result2=1;
else
    result2=0;
end
if entropy<Entropy(end);%get the difference
    result3=1;
else
    result3=0;
end
result=result1+result2+result3;
differnce=(entropy-Entropy(1))-(Entropy(2)-Entropy(3));%两次概率密度变化，第二次变化更大
