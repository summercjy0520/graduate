...
KCC=[SSK;Q];%小聚类的序列集
KC=[KC;SSK];%合并路径上的所有序列
KC=unique(KC,'rows'); %去掉重复元素
entropy=kentropy(KC,'lin_kernel',0); %计算熵，线性核
% TEMP=[TEMP;entropy];
% Eth=max(TEMP)-TEMP(1);%get the difference

if Entropy(end-1)<Entropy(end);%get the difference
   result1=1;
else
   result1=0;
end
% if Entropy(2)<Entropy(3);%get the difference
%    result4=1;
% else
%     result4=0;
% end
if Entropy(1)<Entropy(2);%get the difference
   result2=1;
else
    result2=0;
end
if entropy<Entropy(1);%get the difference
    result3=1;
else
    result3=0;
end
...
% differnce1=(entropy-Entropy(1))-(Entropy(1)-Entropy(2));%两次概率密度变化，第二次变化更大
% differnce2=(Entropy(1)-Entropy(2))-(Entropy(2)-Entropy(3));%两次概率密度变化，第二次变化更大
% differnce=differnce1/differnce2; %计算相对比值
% if zis<4
%     Entropy(zis)=entropy;
%     zis=zis+1;
% else
...
% 第san次计算混合距离
kkk=1;
dd3=NHDTW1(S,Q0,qq/2);
[maxn maxi]=max(dd3);%用于替换，最大值

while (kkk<=kc)
%     jjj=1;%初值
    [minn mini]=min(dd3);%找第一个最小值
    if mini>=qq&&mini<=(mmm-q-s)
    KS(kkk)=mini;%记录对应的行
%     MIN=[MIN,minn]; %记录对应的距离值
    dd3(mini)=maxn;
