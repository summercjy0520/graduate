...
[TXT,TX,RAW] = xlsread('C:\Documents and Settings\Administrator\桌面\cjy\MIToolbox2.11\LS-SVMLab v1.7(R2006a-R2009a)\data\flows.xlsx');
% Xt=[2210,2570,2910,3250,3420,3390,3100,]
% TXT=str2double(TXT);
...
if VV(t)<0.88
    ...
    YT(t)=1;
    A1(t)=t;
elseif VV(t)>=0.88&&VV(t)<1.16
    TT2(t)=VV(t);
    YT(t)=2;
    A2(t)=t;
elseif VV(t)>=1.16&&VV(t)<1.4
    TT3(t)=VV(t);
    YT(t)=-1;
    A3(t)=t;
else
...
for i=1:ww
    ...
    if diff<0.25
        classR(i)=1;
    elseif diff>=0.25&&diff<0.8
        classR(i)=2;
    elseif diff>=0.8&&diff<1.8
        classR(i)=-1;
    else
        classR(i)=-2;
...
ylabel('Values of flows database');
title('DTW:mae=2.2339,rmse=6.8317');



Xt=TXT(h0+1:H1);
Xt=mapminmax('reverse',Xt,T1);
