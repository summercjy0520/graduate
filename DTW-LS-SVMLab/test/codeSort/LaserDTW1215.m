...
[ed1,ed2]=NHDTWnew1215(SSS,SSQ);
for f=1:k
%     cjy1(f)=NH(SSS(f,:),SSQ,(d0-1)/2);    
%     cjy1(f)=0;
    cjy2(f)=abs(SSY(f)-mediann)/ymediann;
    if (ed1(f)==0||ed2(f)==0||cjy2(f)==0)
         cjy(f)=(exp((-1/2)*ed1(f)+(-1/2)*ed2(f))+exp((-1/2)*cjy2(f)));%*(1/2)
    else
         numda2=ed1(f)*(log(ed2(f)/cjy2(f))-1)+log(ed1(f)/cjy2(f))-log(ed2(f)/cjy2(f));
         if abs(numda2/abs(ed1(f)*ed2(f)-cjy2(f)*(ed1(f)+ed2(f))))<1
             numda2=abs(numda2/abs(ed1(f)*ed2(f)-cjy2(f)*(ed1(f)+ed2(f))));
         else
             numda2=abs(abs(ed1(f)*ed2(f)-cjy2(f)*(ed1(f)+ed2(f)))/numda2);
         end
         numda1=numda2*ed2(f)+log(ed1(f)/cjy2(f))-log(ed2(f)/cjy2(f));
         if abs(numda1/ed1(f))<1
             numda1=abs(numda1/ed1(f));
         else
             numda1=abs(ed1(f)/numda1);
         end
%          numda1=min(numda1/ed1(f),ed1(f)/numda1);
         cjy(f)=exp((-numda1)*ed1(f))+exp((-numda2)*ed2(f))+exp((-(1-numda1-numda2))*cjy2(f));
    end
end
