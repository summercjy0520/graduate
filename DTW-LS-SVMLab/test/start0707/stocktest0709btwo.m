...
V(ismember(V,zeros(1,qq),'rows')==1,:)=[];
S0=S;
...
[gamt,sigt2] = tunelssvm({SS,HY,'f',[],[],'RBF_kernel'},'simplex',...
'crossvalidatelssvm',{10,'mae'});

% Prediction of the next 100 points
[alphat,bt] = trainlssvm({SS,HY,'f',gamt,sigt2,'RBF_kernel'});
%predict next 100 points
tempx = predict({SS,HY,'f',gamt,sigt2,'RBF_kernel'},Xs,1);
...
%        end
    ...
    tempy=max(abs(tempx),abs(mean(temp0)));
    if Ytest==1
         prediction(zis)=TXT(h0+zis-1)+tempy;
    else
         prediction(zis)=TXT(h0+zis-1)-tempy;
    end
else
    ...
    tempy=min(abs(tempx),abs(mean(temp0)));
    if Ytest==1
       prediction(zis)=TXT(h0+zis-1)+tempy;
    else
       prediction(zis)=TXT(h0+zis-1)-tempy;
