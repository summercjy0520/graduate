...
%     DIFF3(tt)=TXT(t-2)-TXT(t-3);
    if DIFF1(tt)>=0
            if TT(tt)>=0.01
               ...
               YT(tt)=1;
               A1(tt)=tt;
            elseif TT(tt)>=0&&TT(tt)<0.01
                TT2(tt)=TT(tt);
                YT(tt)=2;
                A2(tt)=tt;
            elseif TT(tt)>=-0.01&&TT(tt)<0
                TT3(tt)=TT(tt);
                YT(tt)=3;
                A3(tt)=tt;
            else
...
[gamt,sigt2] = tunelssvm({SSS1,V30,'c',[],[],'lin_kernel'},'simplex',...
'crossvalidatelssvm',{10,'mae'});

% Prediction of the next 100 points
[alphat,bt] = trainlssvm({SSS1,V30,'c',gamt,sigt2,'lin_kernel'});
%predict next 100 points
Ytest2 = simlssvm({SSS1,V30,'c',gamt,sigt2,'lin_kernel'},{alphat,bt},Xs');
...
for i=1:ww
    ...
    diff2=TXT(h0+i)-TXT(h0+i-2);
    if diff2>=0
            if diff>=0.01
               classR(i)=1;
            elseif diff>=0&&diff<0.01
                classR(i)=2;
            elseif diff>=-0.01&&diff<0
                classR(i)=3;
            elseif diff<-0.01
                classR(i)=4;
            end
