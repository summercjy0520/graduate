function prediction = predict(model,Xt,TT,n, simfct, args)
...
% % closed loop
% eval('for i=1:n, if i>1,Xt=[ prediction(i+1:xdelays); prediction(xdelays+i-1);TT(i-1)], prediction(xdelays+i) = feval(simfct,model, Xt),else  prediction(xdelays+i) = feval(simfct,model, Xt)'',args); end',...
%      'for i=1:n, if i>1,Xt=[ prediction(i+1:xdelays); prediction(xdelays+i-1);TT(i-1)], prediction(xdelays+i) = feval(simfct,model, Xt),else  prediction(xdelays+i) = feval(simfct,model, Xt)''); end');
for i=1:n
    if i>1
        Xt=[ prediction(i+1:xdelays); prediction(xdelays+i-1);TT(i-1)];
        prediction(xdelays+i) = simlssvm(model, Xt');
    else
        prediction(xdelays+i) = simlssvm(model, Xt');
    end
end
