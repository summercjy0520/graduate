...
for k = 1:NT,
    ...
    %    TV(k) = Tac;
    for l = 1:NI,%对应于neighbour()函数，寻找临近点
        % choose new coordinates and compute选择新的坐标并计算
        % the function to minimize
        r = tan(pi*(rand(pdim,pnum)-0.5));%pi是常数3.1416
        % ****************** Wrapping ***************
        ...
        for i=1:pnum
            ...
            if (en(i)-e0(i)) < 0,
                % accept
                p0(:,i) = pn(:,i); e0(i) = en(i);%能量函数和状态函数初始化
                if e0(i) < be0, %be0设为能量e0的最小值
                    be0 = e0(i); 
                    bp0 = p0(:,i); %转化为对应的状态值
%                     if OPT.print == 1,
%                         fprintf('v%e %d\n',min(e0),k);
