...
if model.preprocess(1) == 'p', 
  model.prestatus='changed';%前状态
else
    model.prestatus='ok'; 
end

%
% initiate datapoint selector启动数据点选择
%
model.xtrain = X;
model.ytrain = Y;
...
%
% regularisation term and kenel parameters 正规化期限和kenel参数
%
if(gam<=0), error('gam must be larger then 0');end
model.gam = gam;

%
% initializing kernel type初始化核类型
%
try model.kernel_type = kernel_type; catch, model.kernel_type = 'RBF_kernel';end
if sig2<=0,
...
% in the class labels潜变量或类的标签
model.latent = 'no';

% coding type used for classification用于分类编码类型
model.code = 'original';
