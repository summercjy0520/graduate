...
%         grain(*)   : The grain of the grid evaluated to compose the surface (by default 50)网格的晶粒评估组成的表面（默认50）
%         seldims(*) : The principal inputs one wants to span a grid (by default [1 2])
% 
% See also:
%   trainlssvm, simlssvm.
...
% reconstruct the original support vectors ...重构原始支持向量。。。好像没变诶
[osvX,osvY] = postlssvm(model,model.xtrain(:,1:model.x_dim),model.ytrain(:,1:model.y_dim));


%
% define the principal dimensions one plots 在点上定义主要维数
%
if (model.x_dim>2) 
  % plotted principal dimensions绘制的主要尺寸
...
if model.type(1)=='c', % 'classification' 
  ...
  %%%%%%%%%%%%%%%%%%
    ...
    xrange1 = xmin1:(xmax1-xmin1)/grain:xmax1;%均分坐标轴
    xrange2 = xmin2:(xmax2-xmin2)/grain:xmax2;
    [XX,YY] = meshgrid(xrange1,xrange2);  %确定绘图坐标格
    Xt = [reshape(XX,prod(size(XX)),1) reshape(YY,prod(size(YY)),1)];
    xsteps = length(xrange1);
    ysteps = length(xrange2); % 画外面的大格子？？！！
    
    
    
    %
    % simulate the points模拟点
    %
    restdim = setdiff(1:model.x_dim, princdim);%setdiff差集。
    rest = zeros(size(Xt,1),model.x_dim-2);
    Xt = [Xt rest];  %扩充矩阵？？算扩围？不算啦
    [ZZ,ff,model] = simlssvm1(model,Xt(:,[princdim restdim]));
    if min(ZZ)==max(ZZ), warning('Simulation over the input space results in only one class...'); end
    
    %
    % for plotting, the categorical format is required为绘制所要求的分类格式
    %
...
elseif model.type(1)=='f',
  ...
  % Determine plot limits   确定横坐标的最大值与最小值
  ...
  elseif and(model.x_dim==1,model.y_dim==1) || length(princdim)==1,
    ...
    plot(Yh,'r-');%画出坐标轴已经对应线性函数
    hold all; 
    plot(osvY,'.');%lineseries第30行进入第二次lineseries并且画出图形。。。根据osvY
    xlabel('time');%X轴标签。
    ylabel('Y');%Y轴标签。
