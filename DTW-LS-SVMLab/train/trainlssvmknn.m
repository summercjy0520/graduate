function [model,b,X,Y]  = trainlssvmknn(model,M)
...
%
if (iscell(model)),
    model = initlssvm(model{:});%这里进来的X\Y是不一样的，因此要重新初始化
end
...
if ~((strcmp(model.kernel_type,'RBF_kernel') && length(model.kernel_pars)>=1) ||...
        (strcmp(model.kernel_type,'Lap_kernel') && length(model.kernel_pars)>=0) ||...
...
%
% set & control input variables and dimensions 变量及维数
%
...
% call lssvmMATLAB.m
%
% model = lssvmMATLABknn(model,M);
model = lssvmMATLABknnsai(model,M);
% model = lssvmMATLABknneksai(model,M);
