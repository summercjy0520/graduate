...
% Automatic Feature Extraction by Nystr锟絤 method自动特征提取
%
%
% >> features = AFE(X, kernel, sig2, Xt)
%
% Description
% Using the Nystr锟絤 approximation method, the mapping of data to
...
% matrix. The Matlab ('eigs') or Nystr锟絤's ('eign') approximation
...
if nargin<=7,
  omega = kernel_matrix(Xs, kernel, kernel_pars);%核矩阵。。。内积空间
  if strcmp(type,'eig'),
    [eigvec,eigvals] = eig(omega+2*eye(size(omega,1))); % + jitter factor加入抖动的因素
...
if strcmp(kernel,'RBF_kernel')
    ...
    omegaN = exp(-omegaN./kernel_pars);%也可以对应到RBF的公式，但是意义是什么？？？？
...
features = repmat((1 ./ sqrt(eigvals))',N,1).*features;%复制     提取特征值！！！
