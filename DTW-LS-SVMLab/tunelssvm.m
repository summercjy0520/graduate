...
if strcmp(costfun,'crossvalidatelssvmpinv') || strcmp(costfun,'crossvalidatelssvm') || strcmp(costfun,'rcrossvalidatelssvm') || strcmp(costfun,'crossvalidatesparselssvm')
...
n = size(X,1);
if flag==0 % otherwise no permutation for correlated errors否则没有为相关误差的置换
    if L==n, p = 1:n; else p = randperm(n); end
    X = X(p,:);
    Y = Y(p,:);
    clear i p
end
% calculate help kernel matrix of the support vectors en training data计算支持向量的帮助下核矩阵连接的训练数据
if strcmp(kernel,'RBF_kernel')
    omega = sum(X.^2,2)*ones(1,n);
    omega = omega+omega'-2*(X*X');
elseif strcmp(kernel,'Lap_kernel')
    omega = sum(X.^2,2)*ones(1,n);
    omega = omega+omega'-2*(X*X');
    omega=sqrt(omega); %取模
