function classifier = lapsvmpss(options,data)
    ...
    if gamma_I==0 % SVM (sparse solution)
        ...
        if options.NewtonCholesky
            if t==1
                ...
                % update the Cholesky factorization of the Hessian 分解
                sv_diff=~(sv&sv_prev);
...
while 1
    ...
    % compute new precgradient and objective   新的梯度与对象
    out=sparse([],[],[],n,1,l);
    out(labeled)=Kalpha(labeled)+b;
    ...
    Kgn=data.K*gn; % multiply by the preconditioner 繁殖
...
if lr<0, lr=0; return, end % converged 汇集
